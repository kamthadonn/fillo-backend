// intelligence_learn.js — /api/intelligence
// Routes:
//   POST /learn   — called after scan/draft/spotlight to update learned patterns
//   GET  /profile — returns full venue intelligence profile for dashboard init
//   POST /refresh — called on login to trigger a fresh intelligence update
...
router.get('/profile', authRequired, async (req, res) => {
  try {
    ...
    const profile = await getVenueIntelligence(venue.id, req.user.userId);
...
router.post('/refresh', authRequired, async (req, res) => {
  ...
  setImmediate(async () => {
    try {
      ...
      // Always pull if no profile, or older than 6 hours
      if (!intel || hoursSince > 6) {
        ...
        await runDeepPull({ ...venue, userId: req.user.userId, plan: req.user.plan || 'starter' });
