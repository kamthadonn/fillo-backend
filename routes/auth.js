...
router.post('/signup', async (req, res) => {
  try {
    ...
    const { data: user, error } = await supabase.from('users').insert({
      email, password_hash: hash, name: name || '',
      plan: userPlan, billing: billing || 'monthly',
      status: 'pending_payment', created_at: new Date().toISOString()
    }).select().single();
...
router.post('/login', async (req, res) => {
  try {
    ...
    const { data: teamMembership } = await supabase.from('team_members').select('owner_id, venue_id, role').eq('member_id', user.id).eq('status', 'active').maybeSingle();
    const ownerId = teamMembership?.owner_id || null;
    const venueId = teamMembership?.venue_id || null;
    const role    = teamMembership?.role || 'owner';
    if (invite_token) {
      await supabase.from('team_members').update({ member_id: user.id, status: 'active', invite_token: null }).eq('invite_token', invite_token);
    }
    ...
    res.json({ success: true, token, user: { id: user.id, email: user.email, name: user.name, plan, role, ownerId, venueId } });
  } catch (err) {
...
router.get('/me', async (req, res) => {
  try {
    ...
    const supabase = getSupabase();
    const { data: user, error } = await supabase.from('users').select('id, email, name, plan, billing, status, created_at').eq('id', decoded.userId).single();
    if (error || !user) return res.status(404).json({ error: 'User not found' });
    res.json({ success: true, user: { ...user, role: decoded.role || 'owner', ownerId: decoded.ownerId || null, venueId: decoded.venueId || null } });
  } catch (err) {
