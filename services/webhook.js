...
async function getWebhookConfig(venueId) {
  const { data } = await getSupabase()
    .from('venues').select('webhook_url, webhook_secret').eq('id', venueId).single();
  return data || {};
}

async function saveWebhookConfig(venueId, { webhookUrl, webhookSecret }) {
  const { error } = await getSupabase()
    .from('venues').update({ webhook_url: webhookUrl, webhook_secret: webhookSecret }).eq('id', venueId);
  return !error;
}
