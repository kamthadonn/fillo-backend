...
app.use(cors({
  origin: (origin, callback) => {
    ...
    // Allow any fillo.tech subdomain (white label, branded dashboards)
    if (origin.endsWith('.fillo.tech')) return callback(null, true);
...
useRoute(app, '/api/usage',          './routes/usage');
...
useRoute(app, '/api/capcut',        './routes/capcut');
...
cron.schedule('30 */6 * * *', async () => {
  ...
  try {
    ...
    for (const venue of venues) {
      ...
      // Respect user's auto-scan preference — if they toggled it off, skip
      if (venue.auto_scan === false) {
        console.log(`[Autopilot] ${venue.name} — auto-scan disabled by user, skipping`);
        continue;
      }
